...
echo "=== Submitting FT No-Mixup Jobs ==="
echo ""

# ── FACED ─────────────────────────────────────────────────────────────────

# 1. FACED — no pooling, binary, no mixup
JOB1=$(sbatch --job-name="ft-faced-bin-nm" \
    slurm/run_experiment.sh $MODULE \
    --dataset faced --task binary --no-mixup)
echo "1/8  faced no-pool binary nomixup:   $JOB1"

# 2. FACED — no pooling, 9-class, no mixup
JOB2=$(sbatch --job-name="ft-faced-9cl-nm" \
    slurm/run_experiment.sh $MODULE \
    --dataset faced --task 9-class --no-mixup)
echo "2/8  faced no-pool 9-class nomixup:  $JOB2"

# ── THU-EP ────────────────────────────────────────────────────────────────

# 5. THU-EP — no pooling, binary, no mixup
JOB5=$(sbatch --job-name="ft-thuep-bin-nm" \
    slurm/run_experiment.sh $MODULE \
    --dataset thu-ep --task binary --no-mixup)
echo "5/8  thu-ep no-pool binary nomixup:   $JOB5"

# 6. THU-EP — no pooling, 9-class, no mixup
JOB6=$(sbatch --job-name="ft-thuep-9cl-nm" \
    slurm/run_experiment.sh $MODULE \
    --dataset thu-ep --task 9-class --no-mixup)
echo "6/8  thu-ep no-pool 9-class nomixup:  $JOB6"

echo ""
echo "All 8 no-mixup jobs submitted. Monitor with: squeue --me"
echo "Logs will be in: slurm/logs/"
