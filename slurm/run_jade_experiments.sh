...
echo "=== Submitting JADE Jobs (Full FT alpha sweep -- FACED only) ==="
echo ""
...
# Full FT + SupCon alpha sweep: FACED, binary + 9-class, alpha in {0.1, 0.2, 0.3, 0.7, 0.8, 0.9}, CV only.

JOB=$(sbatch --job-name="jade-faced-bin-fft-a01" \
    slurm/run_experiment.sh $MODULE \
    --dataset faced --task binary --fullft --alpha 0.1)
echo " 1/12 faced binary  fullft a=0.1 CV: $JOB"

JOB=$(sbatch --job-name="jade-faced-bin-fft-a02" \
    slurm/run_experiment.sh $MODULE \
    --dataset faced --task binary --fullft --alpha 0.2)
echo " 2/12 faced binary  fullft a=0.2 CV: $JOB"

JOB=$(sbatch --job-name="jade-faced-bin-fft-a03" \
    slurm/run_experiment.sh $MODULE \
    --dataset faced --task binary --fullft --alpha 0.3)
echo " 3/12 faced binary  fullft a=0.3 CV: $JOB"

JOB=$(sbatch --job-name="jade-faced-bin-fft-a07" \
    slurm/run_experiment.sh $MODULE \
    --dataset faced --task binary --fullft --alpha 0.7)
echo " 4/12 faced binary  fullft a=0.7 CV: $JOB"

JOB=$(sbatch --job-name="jade-faced-bin-fft-a08" \
    slurm/run_experiment.sh $MODULE \
    --dataset faced --task binary --fullft --alpha 0.8)
echo " 5/12 faced binary  fullft a=0.8 CV: $JOB"

JOB=$(sbatch --job-name="jade-faced-bin-fft-a09" \
    slurm/run_experiment.sh $MODULE \
    --dataset faced --task binary --fullft --alpha 0.9)
echo " 6/12 faced binary  fullft a=0.9 CV: $JOB"

JOB=$(sbatch --job-name="jade-faced-9cl-fft-a01" \
    slurm/run_experiment.sh $MODULE \
    --dataset faced --task 9-class --fullft --alpha 0.1)
echo " 7/12 faced 9-class fullft a=0.1 CV: $JOB"

JOB=$(sbatch --job-name="jade-faced-9cl-fft-a02" \
    slurm/run_experiment.sh $MODULE \
    --dataset faced --task 9-class --fullft --alpha 0.2)
echo " 8/12 faced 9-class fullft a=0.2 CV: $JOB"

JOB=$(sbatch --job-name="jade-faced-9cl-fft-a03" \
    slurm/run_experiment.sh $MODULE \
    --dataset faced --task 9-class --fullft --alpha 0.3)
echo " 9/12 faced 9-class fullft a=0.3 CV: $JOB"

JOB=$(sbatch --job-name="jade-faced-9cl-fft-a07" \
    slurm/run_experiment.sh $MODULE \
    --dataset faced --task 9-class --fullft --alpha 0.7)
echo "10/12 faced 9-class fullft a=0.7 CV: $JOB"

JOB=$(sbatch --job-name="jade-faced-9cl-fft-a08" \
    slurm/run_experiment.sh $MODULE \
    --dataset faced --task 9-class --fullft --alpha 0.8)
echo "11/12 faced 9-class fullft a=0.8 CV: $JOB"

JOB=$(sbatch --job-name="jade-faced-9cl-fft-a09" \
    slurm/run_experiment.sh $MODULE \
    --dataset faced --task 9-class --fullft --alpha 0.9)
echo "12/12 faced 9-class fullft a=0.9 CV: $JOB"

echo ""
echo "All 12 JADE fullft jobs submitted. Monitor with: squeue -u $USER"
