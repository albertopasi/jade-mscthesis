"""
train_jade.py — Main execution script for JADE (Joint Alignment and Discriminative Embedding).

Two-stage pipeline:
  Stage 1 (LP warmup): frozen encoder, train cls_query_token + linear head (CE only).
  Stage 2 (FT):        LoRA adapters (or full FT) + joint CE + SupCon loss.
...
Run with:
    ...
    # Different projection input
    uv run python -m src.approaches.jade.train_jade --dataset faced --supcon-repr mean
...
def run_fold_jade(
    ...
    """
    Run one fold: LP warmup (CE only) → FT with LoRA/full FT + joint CE + SupCon.
    """
    ...
    model = ReveClassifierJADE(
        ...
        proj_dim=cfg.supcon_proj_dim,
        proj_hidden=cfg.supcon_proj_hidden,
    ...
    lp_result = train_stage_jade(
        ...
        use_supcon=False,  # CE only in LP warmup
    )
    ...
    ft_result = train_stage_jade(
        ...
        weight_decay=cfg.weight_decay,
        use_mixup=False,  # mixup always off when SupCon active
        use_amp=cfg.use_amp,
        n_classes=cfg.num_classes,
        device=DEVICE,
    ...
    if ft_result.get("best_state"):
        ...
        print(f"Projection head saved → {proj_path}")
...
def parse_args() -> JADEConfig:
    ...
    parser = argparse.ArgumentParser(
        description="JADE: Joint Alignment and Discriminative Embedding (CE + SupCon) on REVE"
    )
    ...
    return JADEConfig(
        ...
        supcon_alpha=args.alpha,
        supcon_temperature=args.temperature,
        supcon_proj_dim=args.proj_dim,
        supcon_proj_hidden=args.proj_hidden,
...
def main() -> None:
    ...
    print(
        f"SupCon: alpha={cfg.supcon_alpha}  tau={cfg.supcon_temperature}  "
        f"repr={cfg.supcon_repr}  proj_dim={cfg.supcon_proj_dim}  "
        f"proj_hidden={cfg.supcon_proj_hidden}"
    )
    if cfg.full_ft:
    ...
    if cfg.reve_split:
        ...
        summary_path = OUTPUT_DIR / (
            ...
            f"{cfg.pool_tag}_{ft_tag}_{cfg._supcon_tag}_revesplit.json"
        )
