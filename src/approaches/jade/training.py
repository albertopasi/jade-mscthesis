...
def train_stage_jade(
    ...
    *,
    ...
    print(f"\n{'─' * COL_W}")
    supcon_label = f"  |  SupCon alpha={supcon_alpha} tau={supcon_temperature}" if use_supcon else ""
    print(f"  Stage: {stage_name}  |  lr={lr}  |  max_epochs={max_epochs}  |  grad_clip={grad_clip}{supcon_label}")
    header = (
