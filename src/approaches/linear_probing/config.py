...
DATA_ROOT       = PROJECT_ROOT / "data" / "thu ep" / "preprocessed_v2"
EMBEDDINGS_DIR  = PROJECT_ROOT / "data" / "thu ep" / "embeddings_v2"
...
class LPConfig:
    ...
    def run_name(self, fold_idx: int, gen_seed: int | None = None) -> str:
        ...
        return f"lp_{self.data_tag}_{self.task_mode}_{self.window_tag}_{self.pool_tag}{self.norm_tag}{gen}_fold_{fold_idx}"

    def group_name(self) -> str:
        gen = "_gen" if self.generalization else ""
        return f"lp_{self.data_tag}_{self.task_mode}_{self.window_tag}_{self.pool_tag}{self.norm_tag}{gen}"
