...
Provides:
  ...
  - LinearProber: Lightning Module that trains a single linear layer on top of
    pre-computed REVE embeddings. Head: RMSNorm → Dropout → Linear (matching
    the official REVE downstream classifier architecture).
"""
...
class LinearProber(L.LightningModule):
    """
    Classification head trained on top of pre-computed REVE embeddings.

    Architecture (matching official REVE downstream classifier):
        Input: (B, embed_dim) float32 embeddings
        → RMSNorm(embed_dim) → Dropout(dropout) → Linear(embed_dim, num_classes)
    ...
    Args:
        ...
        lr:                 Peak learning rate (default 5e-3).
        dropout:            Dropout rate in classification head (default 0.05).
        warmup_epochs:      Number of linear warmup epochs (default 3).
        scheduler_patience: ReduceLROnPlateau patience in epochs (default 6).
    """
    ...
    def __init__(
        ...
        # Classification head: RMSNorm → Dropout → Linear
