...
class WarmupCallback(Callback):
    ...
    During warmup the LR ramps linearly from ~0 to base_lr.  After warmup the
    optimizer LR is reset to base_lr and ReduceLROnPlateau takes over.
    """
    ...
    def on_train_epoch_start(self, trainer: L.Trainer, pl_module: L.LightningModule) -> None:
        ...
        if epoch < self.warmup_epochs:
            # Linear ramp: epoch 0 → 1/W, epoch 1 → 2/W, ..., epoch W-1 → 1.0
...
def run_fold(
    ...
    warmup_cb = WarmupCallback(
        warmup_epochs=cfg.warmup_epochs, base_lr=cfg.lr,
    )
