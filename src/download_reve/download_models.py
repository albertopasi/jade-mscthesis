...
def download_reve_model(
    ...
    if model_name not in REVE_MODELS:
        raise ValueError(
            f"Unknown model: {model_name}. Available models: {list(REVE_MODELS.keys())}"
        )
...
def load_local_reve_model(
    ...
    if not model_path.exists():
        raise FileNotFoundError(
            f"Model not found at {model_path}. Run download_reve_model('{model_name}') first."
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Download REVE models from Hugging Face")
    parser.add_argument(
        "--save-dir",
        type=Path,
