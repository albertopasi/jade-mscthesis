...
for i, pos_name in enumerate(all_positions):
    ...
    positions_dict[pos_name] = {"x": float(coord[0]), "y": float(coord[1]), "z": float(coord[2])}

# Save to JSON
output_path = Path(__file__).parent / "electrodes_pos" / "reve_all_positions.json"
...
for name in standard_10_20:
    if name in positions_dict:
        ...
        print(
            f"{name:<20} {coords['x']:<12.2f} {coords['y']:<12.2f} {coords['z']:<12.2f} {'✓':<12}"
        )
    else:
