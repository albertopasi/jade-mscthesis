...
def visualize_sample(data: np.ndarray, sample_idx: int = 0) -> None:
    ...
    fig.suptitle(
        f"FACED EEG - All {N_CHANNELS} Channels (Sample {sample_idx})",
        fontsize=14,
        fontweight="bold",
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--subject", "-s", type=int, default=0, help="Subject ID (0-indexed, default: 0)"
    )
    parser.add_argument(
        "--sample", "-n", type=int, default=0, help="Sample index within subject (0-27, default: 0)"
    )
    ...
    try:
        ...
        print(
            f"Visualizing sample {args.sample} of subject {args.subject} ({args.sample}/{data.shape[0] - 1})"
        )
