...
def explore_ratings():
    ...
    if ratings is not None:
        ...
        for stim_idx in range(ratings.shape[1]):
            ...
            row = (
                f"  {stim_idx + 1:4d}  |"
                + "".join([f"  {item_means[i]:5.2f}" for i in range(12)])
                + f" |  {overall_mean:5.2f}"
            )
            print(row)
...
def explore_subject_eeg(subject_id: int = 1):
    ...
    for key in mat.keys():
        ...
        if isinstance(value, np.ndarray) and value.ndim >= 2:
            ...
            if value.ndim == 2:
                print(
                    f"    Likely interpretation: {value.shape[0]} channels x {value.shape[1]} samples"
                )
                print(f"    Or: {value.shape[0]} trials x {value.shape[1]} features")
            elif value.ndim == 3:
                print(
                    f"    Likely interpretation: {value.shape[0]} trials x {value.shape[1]} channels x {value.shape[2]} samples"
                )
