...
def check_low_variance(eeg: np.ndarray, percentile_threshold: float = 1.0) -> Dict:
    ...
    return {
        ...
        "low_variance_channels": [
            ALL_CHANNELS[i] for i in np.where(low_var_mask)[0] if i < len(ALL_CHANNELS)
        ],
        ...
        for start, end in zip(starts, ends):
            ...
            if length >= min_length:
                constant_segments.append(
                    {
                        "channel": ALL_CHANNELS[ch_idx]
                        if ch_idx < len(ALL_CHANNELS)
                        else f"ch{ch_idx}",
                        "channel_idx": ch_idx,
...
def inspect_stimulus(subject_id: int, stimulus_idx: int, verbose: bool = True) -> Dict:
    """
...
def print_inspection_report(results: Dict):
    ...
    print("\n" + "=" * 70)
    print(
        f"DATA QUALITY REPORT: Subject {results['subject_id']}, Stimulus {results['stimulus_idx']}"
    )
    print("=" * 70)
    ...
    for i in range(min(5, len(ALL_CHANNELS))):
        print(
            f"   {ALL_CHANNELS[i]:<8} {stats['channel_means'][i]:>10.2f} "
    ...
    status = (
        "✓"
        if not z["all_zeros"] and z["zero_percent"] < 10
        else "⚠"
        if z["zero_percent"] < 50
        else "✗"
    )
    ...
    if cs["has_constant_segments"]:
        ...
        for seg in cs["constant_segments"][:3]:  # Show first 3
            print(
                f"      {seg['channel']}: {seg['length_seconds']:.2f}s "
    ...
    print(
        f"{status} Outliers (±10σ): {ol['outlier_percent']:.3f}% "
...
def scan_all_subjects(verbose: bool = False, check_all_stimuli: bool = False) -> List[Dict]:
    """
    ...
    if issues_found:
        ...
        for item in issues_found:
            print(
                f"  Subject {item['subject_id']}: {len(item['stimuli_with_issues'])} problematic stimuli"
            )

    return issues_found


def scan_extreme_artifacts(std_threshold: float = 500.0, verbose: bool = True) -> List[Dict]:
    """
    ...
    for subject_id in range(1, 81):
        ...
        for stim in range(28):
            ...
            if std > std_threshold:
                ...
                ch_name = (
                    ALL_CHANNELS[worst_ch_idx]
                    if worst_ch_idx < len(ALL_CHANNELS)
                    else f"ch{worst_ch_idx}"
                )
        ...
        if bad_stimuli:
            ...
            if verbose:
                print(
                    f"Subject {subject_id:2d}: ⚠ {len(bad_stimuli):2d}/28 stimuli affected, max_std={max_std:>10.0f}"
                )
        else:
            if verbose:
    ...
    if problematic:
        ...
        for stim_info in worst["affected_stimuli"][:10]:  # Show first 10
            rng = stim_info["range"]
            print(
                f"   {stim_info['stimulus']:>4d} {stim_info['std']:>12.0f} "
...
def inspect_all_stimuli(subject_id: int) -> List[Dict]:
    ...
    for i, result in enumerate(results):
        ...
        print(
            f"{i:<6} {stats['global_mean']:>10.2f} {stats['global_std']:>10.2f} "
...
def main():
    ...
    parser.add_argument("-s", "--subject", type=int, help="Subject ID (1-80)")
    parser.add_argument(
        "-st", "--stimulus", type=int, default=0, help="Stimulus index (0-27), default: 0"
    )
    parser.add_argument("--scan-all", action="store_true", help="Scan all 80 subjects for issues")
    parser.add_argument(
        "--all-stimuli", action="store_true", help="Check all 28 stimuli for specified subject"
    )
    parser.add_argument(
        "--deep-scan",
        action="store_true",
