...
Usage:
    ...
    uv run python -m src.exploration.thu_ep.vis_old --viewer

    # Static plot for specific subject/stimulus
    uv run python -m src.exploration.thu_ep.vis_old --subject 1 --stimulus 0

    # Export comparison images
    uv run python -m src.exploration.thu_ep.vis_old --subject 1 --stimulus 0 --export outputs/comparison.png
...
def plot_comparison_simple(
    ...
    print(
        f"  Preprocessed: {len(prep_channels)} channels at {prep_sfreq} Hz ({prep_eeg.shape[1]} samples)"
    )
    ...
    fig, axes = plt.subplots(
        2, 1, figsize=(20, 16), gridspec_kw={"height_ratios": [height_ratio_raw, 1]}
    )
    ...
    def _update_plot(self):
        ...
        if result[0] is None:
            self.ax_raw.text(
                0.5,
                0.5,
                f"Error loading subject {self.subject_id}",
                ha="center",
                va="center",
                fontsize=14,
                color="red",
            ...
            self.ax_prep.text(
                0.5, 0.5, "Data not available", ha="center", va="center", fontsize=14, color="red"
            )
        ...
        self.ax_raw.set_yticks(
            [(n_raw_channels - 1 - i) * raw_spacing for i in range(n_raw_channels)]
        )
        ...
        self.ax_prep.set_yticks(
            [(n_prep_channels - 1 - i) * prep_spacing for i in range(n_prep_channels)]
        )
        ...
        # Update main title
        self.fig.suptitle(
            "THU-EP EEG Comparison: Raw vs Preprocessed", fontsize=14, fontweight="bold"
        )
