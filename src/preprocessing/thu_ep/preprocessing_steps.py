...
def extract_frequency_band(
    data: np.ndarray, band_index: int, band_name: str = "", verbose: bool = False
) -> np.ndarray:
    """
...
def remove_reference_channels(
    data: np.ndarray, channels_to_remove_indices: List[int], verbose: bool = False
) -> np.ndarray:
    """
    ...
    if verbose:
        ...
        print(
            f"  Removed {removed_count} reference channels: {data.shape[1]} -> {result.shape[1]} channels"
        )

    return result


def downsample_stimuli(
    data: np.ndarray, original_sfreq: float, target_sfreq: float, verbose: bool = False
) -> np.ndarray:
    """
    ...
    if verbose:
        print(
            f"  Downsampled: {original_sfreq} Hz -> {target_sfreq} Hz "
...
def compute_global_statistics(
    ...
    if verbose:
        print(
            f"  Computed global statistics across {n_stimuli} stimuli "
...
def z_normalize_global(
    data: np.ndarray, global_mean: np.ndarray, global_std: np.ndarray, verbose: bool = False
) -> np.ndarray:
    """
    ...
    if verbose:
        print("  Applied global Z-normalization")

    return result


def artifact_clipping(data: np.ndarray, threshold_std: float, verbose: bool = False) -> np.ndarray:
    """
...
def export_subject_npy(
    subject_id: int, data: np.ndarray, output_dir: str, verbose: bool = False
) -> str:
    """
