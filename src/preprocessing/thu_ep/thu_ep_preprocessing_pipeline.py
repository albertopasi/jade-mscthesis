...
Preprocessing steps (matching REVE/FACED downstream convention):
1. Extract broad-band (band index 5 = 0.5-47 Hz)
2. Remove A1, A2 reference channels (32 -> 30 channels)
3. Downsample 250 Hz -> 200 Hz (7500 -> 6000 samples)
4. Export as .npy files (raw µV values, NO z-scoring or clipping)

Amplitude scaling (µV → REVE input range) is handled at load time via
...
class THUEPPreprocessingPipeline:
    ...
    def process_subject(self, filepath: Path) -> Dict:
        ...
        try:
            ...
            if self.config.verbose:
                ...
                print("    Expected: (7500, 32, 28, 6) = (samples, channels, stimuli, bands)")
            ...
            if self.config.is_step_enabled("extract_band"):
                ...
                data = extract_frequency_band(
                    data, self.config.broad_band_index, band_name, verbose=self.config.verbose
                )
                if self.config.verbose:
            ...
            if self.config.is_step_enabled("remove_reference_channels"):
                data = remove_reference_channels(
                    data, self.config.channels_to_remove_indices, verbose=self.config.verbose
                )
                if self.config.verbose:
            ...
            if self.config.is_step_enabled("z_normalize"):
                global_mean, global_std = compute_global_statistics(
                    data, verbose=self.config.verbose
                )
                ...
                data = z_normalize_global(
                    data, global_mean, global_std, verbose=self.config.verbose
                )
                if self.config.verbose:
            ...
            if self.config.is_step_enabled("artifact_clipping"):
                data = artifact_clipping(
                    data, self.config.artifact_threshold_std, verbose=self.config.verbose
                )
                if self.config.verbose:
            ...
            if self.config.is_step_enabled("export_npy"):
                output_file = export_subject_npy(
                    subject_id, data, str(self.config.preprocessed_dir), verbose=self.config.verbose
                )
    ...
    def validate_preprocessed_data(self, subject_id: int) -> Dict:
        ...
        clipping_enabled = self.config.is_step_enabled("artifact_clipping")
        within_clip = bool(
            data.min() >= -self.config.artifact_threshold_std
            and data.max() <= self.config.artifact_threshold_std
        )
        validation = {
            "subject_id": subject_id,
            "shape": data.shape,
            ...
            "std": float(data.std()),
            "within_clip_range": within_clip,
        }

        validation["valid"] = (
            validation["shape_valid"]
            and validation["dtype_valid"]
            and not validation["has_nan"]
            and not validation["has_inf"]
            and (validation["within_clip_range"] if clipping_enabled else True)
        )
